...
std::vector<result> readTXT(std::string fileName)
{
  ...
  float resol;
  while(1){
    in >> ebeam >> ncluster >> nclusterError >> sigma >> sigmaError >> resol;
    result res;
    res.ebeam=ebeam;
    if( (ebeam/5)%2 == 1)continue;
    res.ncluster=ncluster;
    res.nclusterError=nclusterError;
    res.sigma=sigma;
...
void DrawNCLUSTER()
{
  ...
  std::vector<result> resultData=readTXT(std::string("ncluster_nov2012.txt"));
  ...
  for(unsigned int i=0; i<resultData.size(); i++){
    std::cout << resultData.at(i).ebeam << " " << resultData.at(i).ncluster << " " << resultData.at(i).nclusterError << std::endl;
  ...
  std::vector<result> resultFTFP=readTXT(std::string("ncluster_ftfp_bert_hp_pion.txt"));
  ...
  std::vector<result> resultQGSP=readTXT(std::string("ncluster_qgsp_bert_hp_pion.txt"));
  ...
  r1->cd();
  r1->SetSelected(r1);
  r1->SaveAs("../plots/NCLUSTERPIONNOV.pdf");
}

...
std::vector<result> readTXT(std::string fileName)
{
  ...
  float radProfRMS;
  float radProfError;
  float radProfRMSError;
  while(1){
    in >> ebeam >> radProf >> radProfError >> radProfRMS >> radProfRMSError;
    if( (ebeam/5)%2 == 1)continue;
    result res;
    res.ebeam=ebeam;
    res.radProf=radProf;
    res.radProfRMS=radProfRMS;
    res.radProfError=radProfError;
    res.radProfRMSError=radProfRMSError;
    vec.push_back(res);
    if(ebeam==80)break;
  }
  return vec;
...
void DrawRAD()
{
  ...
  TCanvas *r1 = new TCanvas("r1", "RadProf",12,24,550,741);
  gStyle->SetOptStat(0);
  ...
  std::vector<result> resultData=readTXT(std::string("radProf_pi-_Nov2012.txt"));
  ...
  for(unsigned int i=0; i<resultData.size(); i++){
    gre->SetPoint(i,resultData.at(i).ebeam,resultData.at(i).radProf);
    gre->SetPointError(i,0,resultData.at(i).radProfError);
  }
  ...
  std::vector<result> resultFTFP=readTXT(std::string("radProf_pi-_ftfp_bert_hp.txt"));
  ...
  for(unsigned int i=0; i<resultFTFP.size(); i++){
    gre->SetPoint(i,resultFTFP.at(i).ebeam,resultFTFP.at(i).radProf);
    gre->SetPointError(i,0,resultFTFP.at(i).radProfError);
  }
  ...
  std::vector<result> resultQGSP=readTXT(std::string("radProf_pi-_qgsp_bert_hp.txt"));
  ...
  for(unsigned int i=0; i<resultQGSP.size(); i++){
    gre->SetPoint(i,resultQGSP.at(i).ebeam,resultQGSP.at(i).radProf);
    gre->SetPointError(i,0,resultQGSP.at(i).radProfError);
  }
  ...
  gre->SetTitle("Graph");
  for(unsigned int i=0; i<resultQGSP.size(); i++){
    delta=(resultQGSP.at(i).radProf-resultData.at(i).radProf)/resultData.at(i).radProf;
    deltaError=1/resultData.at(i).radProf*
      sqrt(pow(resultQGSP.at(i).radProfError,2) +
    	   pow(resultQGSP.at(i).radProf/resultData.at(i).radProf*resultData.at(i).radProfError,2));
  ...
  gre->SetTitle("Graph");
  for(unsigned int i=0; i<resultFTFP.size(); i++){
    delta=(resultFTFP.at(i).radProf-resultData.at(i).radProf)/resultData.at(i).radProf;
    deltaError=1/resultData.at(i).radProf*
      sqrt(pow(resultFTFP.at(i).radProfError,2) +
    	   pow(resultFTFP.at(i).radProf/resultData.at(i).radProf*resultData.at(i).radProfError,2));
  ...
  r1->cd();
  r1->SetSelected(r1);
  r1->SaveAs("../plots/RADPIONNOV.pdf");
}
...
void DrawRMS()
{
  //=========Macro generated from canvas: r1/Energy
  //=========  (Fri Apr  6 17:14:48 2012) by ROOT version5.28/00g
  TCanvas *r1 = new TCanvas("rms", "RadProfRMS",12,24,550,741);
  gStyle->SetOptStat(0);
  gStyle->SetOptTitle(0);
  gStyle->SetFrameLineWidth(1);
  r1->Range(0,0,1,1);
  ...
  // ------------>Primitives in pad: r1_1
  TPad *r1_1 = new TPad("r1_rms", "RadProfRMS",0.02,0.37,0.95,0.99);
  r1_1->Draw();
  r1_1->cd();
  r1_1->Range(-19,0.01,95,95);
  r1_1->SetFillColor(0);
  ...
  he->GetXaxis()->SetTitleFont(43);
  he->GetXaxis()->SetTitleSize(0); 
  he->GetYaxis()->SetTitle("#sigma_{R} [cm]");
  he->GetYaxis()->SetLabelFont(43);
  he->GetYaxis()->SetTitleSize(30);
  he->GetYaxis()->SetLabelSize(20);
  he->GetYaxis()->SetTitleFont(43);
  ...
  gre->SetMarkerSize(1.2);
  for(unsigned int i=0; i<resultData.size(); i++){
    gre->SetPoint(i,resultData.at(i).ebeam,resultData.at(i).radProfRMS);
    gre->SetPointError(i,0,resultData.at(i).radProfRMSError);
  }
  ...
  gre->SetMarkerStyle(20);
  for(unsigned int i=0; i<resultFTFP.size(); i++){
    gre->SetPoint(i,resultFTFP.at(i).ebeam,resultFTFP.at(i).radProfRMS);
    gre->SetPointError(i,0,resultFTFP.at(i).radProfRMSError);
  }
  ...
  gre->SetMarkerStyle(25);
  for(unsigned int i=0; i<resultQGSP.size(); i++){
    gre->SetPoint(i,resultQGSP.at(i).ebeam,resultQGSP.at(i).radProfRMS);
    gre->SetPointError(i,0,resultQGSP.at(i).radProfRMSError);
  }
  ...
  hd->GetXaxis()->SetTitleSize(30);
  hd->GetXaxis()->SetTitleOffset(2.);
  hd->GetYaxis()->SetTitle("(#Delta#sigma_{R})/#sigma_{R}");
  hd->GetYaxis()->SetLabelFont(43);
  hd->GetYaxis()->SetLabelSize(20);
  hd->GetYaxis()->SetTitleSize(30);
  hd->GetYaxis()->SetTitleOffset(1.7);
  ...
  gre->SetTitle("Graph");
  for(unsigned int i=0; i<resultQGSP.size(); i++){
    delta=(resultQGSP.at(i).radProfRMS-resultData.at(i).radProfRMS)/resultData.at(i).radProfRMS;
    deltaError=1/resultData.at(i).radProfRMS*
      sqrt(pow(resultQGSP.at(i).radProfRMSError,2) +
    	   pow(resultQGSP.at(i).radProfRMS/resultData.at(i).radProfRMS*resultData.at(i).radProfRMSError,2));
    gre->SetPoint(i,resultQGSP.at(i).ebeam,delta);
    gre->SetPointError(i,0,deltaError);
  }
  gre->SetLineWidth(2);
  ...
  gre->SetTitle("Graph");
  for(unsigned int i=0; i<resultFTFP.size(); i++){
    delta=(resultFTFP.at(i).radProfRMS-resultData.at(i).radProfRMS)/resultData.at(i).radProfRMS;
    deltaError=1/resultData.at(i).radProfRMS*
      sqrt(pow(resultFTFP.at(i).radProfRMSError,2) +
    	   pow(resultFTFP.at(i).radProfRMS/resultData.at(i).radProfRMS*resultData.at(i).radProfRMSError,2));
    gre->SetPoint(i,resultFTFP.at(i).ebeam,delta);
    gre->SetPointError(i,0,deltaError);
  }
  gre->SetLineWidth(2);
  ...
  r1->cd();
  r1->SetSelected(r1);
  r1->SaveAs("../plots/RADRMSPION.pdf");
}

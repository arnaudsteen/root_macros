...
void DrawNCLUSTER()
{
  ...
  std::vector<result> resultData=readTXT(std::string("cluster_pi-_AugSep2012.txt"));
  ...
  std::vector<result> resultFTFP=readTXT(std::string("cluster_pi-_ftfp_bert_hp.txt"));
  ...
  std::vector<result> resultQGSP=readTXT(std::string("cluster_pi-_qgsp_bert_hp.txt"));
  ...
  r1->cd();
  r1->SetSelected(r1);
  r1->SaveAs("../plots/NCLUSTERPION.pdf");
}

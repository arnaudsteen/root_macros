...
void DrawLONGI()
{
  ...
  std::vector<result> resultData=readTXT(std::string("longiProf_e-_AugSep2012.txt"));
  ...
  std::vector<result> resultFTFP=readTXT(std::string("longiProf_e-_ftfp_bert_hp.txt"));
  ...
  std::vector<result> resultQGSP=readTXT(std::string("longiProf_e-_qgsp_bert_hp.txt"));
  ...
  r1->cd();
  r1->SetSelected(r1);
  r1->SaveAs("../plots/LONGIELECTRON.pdf");
}
...
void DrawRMS()
{
  ...
  r1->cd();
  r1->SetSelected(r1);
  r1->SaveAs("../plots/LONGIRMSELECTRON.pdf");
}

...
void DrawRAD()
{
  ...
  std::vector<result> resultData=readTXT(std::string("radProf_pi-_AugSep2012.txt"));
  ...
  for(unsigned int i=0; i<resultData.size(); i++){
    ...
    std::cout << resultData.at(i).ebeam << " GeV \t " << resultData.at(i).radProf << " #pm " << resultData.at(i).radProfError << std::endl;
  }

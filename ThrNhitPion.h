...
#ifndef ThrNhitPion_h
#define ThrNhitPion_h
...
#ifdef ThrNhitPion_cxx
ThrNhitPion::ThrNhitPion(const char*input) : fChain(0) 
{
...
ThrNhitPion::~ThrNhitPion()
{
  if (!fChain) return;
  delete fChain->GetCurrentFile();
}

Int_t ThrNhitPion::GetEntry(Long64_t entry)
{
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntry(entry);
}
Long64_t ThrNhitPion::LoadTree(Long64_t entry)
{
...
void ThrNhitPion::Init(TTree *tree)
{
...
void ThrNhitPion::Show(Long64_t entry)
{
...
Int_t ThrNhitPion::Cut(Long64_t entry)
{

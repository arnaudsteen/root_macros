...
#ifndef NhitElectron_h
#define NhitElectron_h
...
class NhitElectron {
 public :
  ...
  NhitElectron(const char* input);
  virtual ~NhitElectron();
...
#ifdef NhitElectron_cxx
NhitElectron::NhitElectron(const char*input) : fChain(0) 
{
...
NhitElectron::~NhitElectron()
{
  if (!fChain) return;
  delete fChain->GetCurrentFile();
}

Int_t NhitElectron::GetEntry(Long64_t entry)
{
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntry(entry);
}
Long64_t NhitElectron::LoadTree(Long64_t entry)
{
...
void NhitElectron::Init(TTree *tree)
{
...
void NhitElectron::Show(Long64_t entry)
{
...
Int_t NhitElectron::Cut(Long64_t entry)
{
...
int Xmin(int E)
{
  ...
  else if(E==30) return 150;
  else if(E==40) return 170;
  else if(E==50) return 250;
  else
...
{
  if(E==10) return 280;
  else if(E==20) return 400;
  else if(E==30) return 500;
  else if(E==40) return 570;
  else if(E==50) return 650;
  else
    std::cout << "Energy = " << E << " GeV IS NOT A CORRECT ENERGY" << std::endl;
  return 0;
}

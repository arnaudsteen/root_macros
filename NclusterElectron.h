...
#ifndef NclusterElectron_h
#define NclusterElectron_h
...
class NclusterElectron {
 public :
  ...
  NclusterElectron(const char* input);
  virtual ~NclusterElectron();
...
#ifdef NclusterElectron_cxx
NclusterElectron::NclusterElectron(const char*input) : fChain(0) 
{
...
NclusterElectron::~NclusterElectron()
{
  if (!fChain) return;
  delete fChain->GetCurrentFile();
}

Int_t NclusterElectron::GetEntry(Long64_t entry)
{
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntry(entry);
}
Long64_t NclusterElectron::LoadTree(Long64_t entry)
{
...
void NclusterElectron::Init(TTree *tree)
{
...
Bool_t NclusterElectron::Notify()
{
...
void NclusterElectron::Show(Long64_t entry)
{
...
Int_t NclusterElectron::Cut(Long64_t entry)
{
...
{
  if(E==10) return 0;
  else if(E==20) return 5;
  else if(E==30) return 5;
  else if(E==40) return 10;
  else if(E==50) return 10;
  else
...
{
  if(E==10) return 50;
  else if(E==20) return 65;
  else if(E==30) return 75;
  else if(E==40) return 80;
  else if(E==50) return 90;
  else
    std::cout << "Energy = " << E << " GeV IS NOT A CORRECT ENERGY" << std::endl;
  return 0;
}
...
{
  if(E==10) return 25;
  else if(E==20) return 30;
  else if(E==30) return 35;
  else if(E==40) return 35;
  else if(E==50) return 40;
  else
    std::cout << "Energy = " << E << " GeV IS NOT A CORRECT ENERGY" << std::endl;
  return 0;
}
...
#endif // #ifdef NclusterElectron_cxx

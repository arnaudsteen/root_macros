...
#ifndef NhitPerThrPion_h
#define NhitPerThrPion_h
...
class NhitPerThrPion {
 public :
  ...
  NhitPerThrPion(const char* input);
  virtual ~NhitPerThrPion();
...
#ifdef NhitPerThrPion_cxx
NhitPerThrPion::NhitPerThrPion(const char*input) : fChain(0) 
{
...
NhitPerThrPion::~NhitPerThrPion()
{
  if (!fChain) return;
  delete fChain->GetCurrentFile();
}

Int_t NhitPerThrPion::GetEntry(Long64_t entry)
{
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntry(entry);
}
Long64_t NhitPerThrPion::LoadTree(Long64_t entry)
{
...
void NhitPerThrPion::Init(TTree *tree)
{
...
void NhitPerThrPion::Show(Long64_t entry)
{
...
Int_t NhitPerThrPion::Cut(Long64_t entry)
{
...
#endif // #ifdef NhitPerThrPion_cxx

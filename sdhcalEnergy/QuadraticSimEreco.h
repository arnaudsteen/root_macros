...
#ifndef QuadraticSimEreco_h
#define QuadraticSimEreco_h
...
double myQuadraticFunc(double x1, double y1, double z1, 
...
class QuadraticSimEreco {
 public :
  ...
  QuadraticSimEreco(const char* input);
  virtual ~QuadraticSimEreco();
  virtual Int_t    Cut(Long64_t entry);
  virtual Int_t    GetEntry(Long64_t entry);
  virtual Long64_t LoadTree(Long64_t entry);
  virtual void     Init(TTree *tree);
  virtual void     Loop(bool withMax);
  virtual TH1D*    FindQuadraticErec(int e);
...
#ifdef QuadraticSimEreco_cxx
QuadraticSimEreco::QuadraticSimEreco(const char*input) : fChain(0) 
{
...
QuadraticSimEreco::~QuadraticSimEreco()
{
  if (!fChain) return;
  delete fChain->GetCurrentFile();
}

Int_t QuadraticSimEreco::GetEntry(Long64_t entry)
{
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntry(entry);
}
Long64_t QuadraticSimEreco::LoadTree(Long64_t entry)
{
...
void QuadraticSimEreco::Init(TTree *tree)
{
...
Bool_t QuadraticSimEreco::Notify()
{
...
void QuadraticSimEreco::Show(Long64_t entry)
{
...
Int_t QuadraticSimEreco::Cut(Long64_t entry)
{
...
#endif // #ifdef QuadraticSimEreco_cxx

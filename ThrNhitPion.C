#define ThrNhitPion_cxx
#include "ThrNhitPion.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <cstring>
...
void ThrNhitPion::ShowerBarycenterCut(std::string runPeriod)
{
...
void Nhit(int E, int thr, std::string runPeriod,std::string model)
{
  char inputFile[200];
  char hName[100];
  char fDataName[200];
  ...
  char axisTitle[50];
  char nameTitle[50];
  sprintf(axisTitle,"%s%d%s","N_{hit",thr+1,"}");
  sprintf(nameTitle,"%s%d","nhit",thr+1);

  CaliceStyle();
  int xmin=0;
  int xmax=0;
  if( thr==0 ){ xmin=Xmin1(E);  xmax=Xmax1(E);}
  if( thr==1 ){ xmin=Xmin2(E);  xmax=Xmax2(E);}
  if( thr==2 ){ xmin=Xmin3(E);  xmax=Xmax3(E);}
  ...
  for(unsigned int i=0; i<n; i++){
    if(thr==0) nhitCalib = data->nhit1.at(i) - coeff1*data->time.at(i) - coeff2*data->time.at(i)*data->time.at(i);
    else if(thr==1) nhitCalib = data->nhit2.at(i) - coeff1*data->time.at(i) - coeff2*data->time.at(i)*data->time.at(i);
    else if(thr==2) nhitCalib = data->nhit3.at(i) - coeff1*data->time.at(i) - coeff2*data->time.at(i)*data->time.at(i);
    hDat->Fill(nhitCalib);
  }
  ...
  ThrNhitPion* simu=new ThrNhitPion(inputFile);
  simu->Loop();  
  sprintf(hName,"%s%d%s","hNhit_",E,"GeV");
  TH1D *hSim=new TH1D(hName,hName,50,xmin,xmax);
  hSim->GetXaxis()->SetTitle(axisTitle);
  ...
  n=simu->nhit1.size();
  for(unsigned int i=0; i<n; i++){
    if(thr==0) hSim->Fill(simu->nhit1.at(i));
    else if(thr==1) hSim->Fill(simu->nhit2.at(i));
    else if(thr==2) hSim->Fill(simu->nhit3.at(i));
  }
  sprintf(fSimuName,"%s%s%s%s","/home/steen/CBFit/histo/",nameTitle,model.c_str(),"_pi-.root");
  if(E==5)
  ...
  sprintf(plotName,"%s%s%s%s%s%d%s%s%s","./plots/",model.c_str(),"/",nameTitle,"_pi-_",E,"GeV_",runPeriod.c_str(),".C");
  can->SaveAs(plotName);
  sprintf(plotName,"%s%s%s%s%s%d%s%s%s","./plots/",model.c_str(),"/",nameTitle,"_pi-_",E,"GeV_",runPeriod.c_str(),".pdf");
  can->SaveAs(plotName);
...
void ProcessPion()
{
  ...
  for(unsigned int i=0; i<eSizeNov; i++){
    Nhit(energyNov[i],0,std::string("Nov2012"),std::string("ftfp_bert_hp"));
    Nhit(energyNov[i],0,std::string("Nov2012"),std::string("qgsp_bert_hp"));
  }
}
